      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-16 animate-fadeIn">
          ...
          <p className="font-garamond text-lg text-muted-foreground max-w-2xl mx-auto">
            Captured moments from quests undertaken, battles won, and bonds forged in the fires of experience
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {galleryImages.map((image, index) => (
            <div
              key={index}
              className="group relative aspect-square overflow-hidden rounded-lg border border-elden-mist shadow-gold hover:shadow-gold-intense transition-all duration-500 cursor-pointer animate-slideUp"
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-elden-dark via-elden-dark/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                <div className="absolute bottom-0 left-0 right-0 p-4">
                  ...
                  <h3 className="font-cinzel text-lg font-bold text-gold mb-1">{image.title}</h3>
                  <p className="font-garamond text-sm text-gold-light">{image.description}</p>
                </div>
              </div>
            </div>
          ))}
